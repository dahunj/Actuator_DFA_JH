...
#define RFID_IP		"192.168.13.40"	// NG	RFID Reader IP
#define RFID_PORT	5000			// RFID Port (Server)

CCarrierRFID_NG g_objCarrierRFID_NG;

IMPLEMENT_DYNAMIC(CCarrierRFID_NG, CWnd)

CCarrierRFID_NG::CCarrierRFID_NG()
{
	m_bConnected = FALSE;
...
BEGIN_MESSAGE_MAP(CCarrierRFID_NG, CWnd)
...
// CCarrierRFID_NG 메시지 처리기입니다.

/////////////////////////////////////////////////////////////////////////////
// Message Handlers

LRESULT CCarrierRFID_NG::OnClientConnect(WPARAM wConnect, LPARAM lParam)
{
	m_bConRequet = FALSE;
...
LRESULT CCarrierRFID_NG::OnClientClose(WPARAM wParam, LPARAM lParam)
{
	m_bConRequet = FALSE;
	m_bConnected = FALSE;
...
LRESULT CCarrierRFID_NG::OnClientReceive(WPARAM wClientIdx, LPARAM lServerPort)
{
...
void CCarrierRFID_NG::Initialize()
{
...
void CCarrierRFID_NG::Terminate()
{
	m_bConnected = FALSE;
	m_Client.Close_Socket();
}

BOOL CCarrierRFID_NG::Send_RFIDRead()
{
#ifndef AJIN_BOARD_USE
	m_bRecvComplete = TRUE;
	m_strCarID.Format("NGCAR%02d", m_nCarSeq++);
	return TRUE;
#endif
	...
	strLog.Format(_T("[RFIDN->] : Load Carrier RFID Read Send."));
	g_objLogFile.Save_BarcodeLog(strLog);
